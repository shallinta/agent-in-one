...
- When Navigator delegates sustained execution, Navigator must explicitly require a completion report to Navigator and instruct Pilot to use pair_message_peer after completing the task.
- When Pilot completes a delegated task, Pilot must send exactly one completion report to Navigator with pair_message_peer. The report must contain the completion status, key results and evidence, and unresolved issues or next steps. A delegated task is not collaboration-complete until that Peer Message succeeds.
...
explicitly require Pilot to report completion to Navigator with pair_message_peer.
...
A delegated task is not collaboration-complete until exactly one completion report
to Navigator succeeds through pair_message_peer. Include completion status, key
results and evidence, and unresolved issues or next steps, while keeping the report
concise when a durable artifact or existing Pair Event can carry the full result.
