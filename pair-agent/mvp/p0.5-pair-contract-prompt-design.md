...
这些 section 对 Navigator 和 Pilot 完全相同。Shared Pair Events 和 Shared Projection 仍位于 Active Role Reminder 之前，使同一 Shared Head 上的两个请求尽可能共享公共缓存前缀。
...
Active Role Reminder 保留现有 user-role message 和 request boundary，只扩充 role guidance。
...
4. Active Role Reminder 仍是 Shared Events/Projection 后的第一个角色差异，并位于 Local History 前；
