...
任意一段文本发生字节变化都会产生新的材料身份，但不代表产品发布了新版本。计算时不做 trim、换行替换或语义 normalization，Request Builder 使用的文本与 digest 输入必须完全相同。代码和文档不再使用“Prompt v2”描述本次变更。
...
2. Prompt material ID 等于 Common System 与两份 role guidance 的 bundle 摘要，任一文本发生字节变化必然改变 ID；
